...
- **Фазовый блинк (phase blink)** — fully predictable after owner playtest feedback: moving → the Wraith dissolves and reappears **200 px along its recent actual movement direction** (the frame-over-frame delta covers keyboard, mouse drag and touch alike); standing still → **the phase opens in place, no teleport**. The first window opens 3 s into the run, not on the spawn frame. A fading **echo clone keeps firing 80%-damage bolts for the whole 2 s window** (behind the ship for in-place phases). Dedicated WebAudio SFX (`playVoidBlink`).
