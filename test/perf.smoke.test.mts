...
// Wall-clock замеры (performance.now) шумят под нагрузкой CPU (CI, параллельная
// сборка): тайминг-тесты получают retry — детерминированный провал всё равно
// упадёт после всех попыток, транзиторный флук пройдёт ретраем. node:test
// применяет retry с Node 18.15, но в установленных @types/node поля ещё нет,
// поэтому опции приведены типом напрямую.
const TIMING_TEST_OPTIONS = { retry: 2 } as import("node:test").TestOptions;

test("поздние волны при максимуме эффектов: производительность и стабильность (билд «Немезида»)", TIMING_TEST_OPTIONS, () => {
...
test("стресс-каскад: массовая гибель со взрывами не ломает лимиты частиц (§14)", TIMING_TEST_OPTIONS, () => {
...
test("ТЗ mythic §18: все 6 мификов + максимальный билд — бюджеты и стабильность", TIMING_TEST_OPTIONS, () => {
  const player = makeInitialPlayer("void_wraith");
  player.level = 45;
  for (const id of [
    "damage_up", "big_bullets", "rapid_fire", "piercing", "explosive", "homing",
